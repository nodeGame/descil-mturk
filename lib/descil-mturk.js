...
try {
    ...
    console.log('An error occurred while loading the configuration file: '
                + conf_path);
...
if (!SERVICEKEY) {
    throw new Error(conf_path + ' does not contain the service key.');
}

if (!PROJECT) {
    throw new Error(conf_path + ' does not contain the project code.');
}

if (!DESCIL_URI) {
    throw new Error(conf_path + ' does not contain the service uri .');
}
